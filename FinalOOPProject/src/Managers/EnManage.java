...
import EnemyC.Bat;
import EnemyC.Enemy;
import EnemyC.Knight;
import EnemyC.Orc;
import EnemyC.Wolf;
import UtilityIG.LoadSave;
import UtilityIG.Utils;
import Obj.PathCords;
import Condition_GUI.InPlay;
import static UtilityIG.Constants.Direction.*;
import static UtilityIG.Constants.Tiles.*;
import static UtilityIG.Constants.Enemies.*;

public class EnManage {
    ...
    public EnManage(InPlay playing, PathCords start, PathCords end) {
...
//		tempMethod();
    ...
    private void loadRoadDirArr() {
        roadDirArr = Utils.GetRoadDirArr(playing.getGame().getTileManage().getTypeArr(), start, end);

    }

//	private void tempMethod() {
//		int[][] arr = Utils.GetRoadDirArr(playing.getGame().getTileManager().getTypeArr(), start, end);
//
...
//				updateEnemyMove(e);
    ...
    private void updateEnemyMoveNew(Enemy e) {
        PathCords currTile = getEnemyTile(e);
        int dir = roadDirArr[currTile.getyCord()][currTile.getxCords()];

        e.move(GetSpeed(e.getEnemyType()), dir);

        PathCords newTile = getEnemyTile(e);
        ...
        if (!isTilesTheSame(currTile, newTile)) {
            ...
            int newDir = roadDirArr[newTile.getyCord()][newTile.getxCords()];
            if (newDir != dir) {
                e.setPos(newTile.getxCords() * 32, newTile.getyCord() * 32);
                e.setLastDir(newDir);
            }
        }
    ...
    private PathCords getEnemyTile(Enemy e) {

        switch (e.getLastDir()) {
            case LEFT:
                return new PathCords((int) ((e.getX() + 31) / 32), (int) (e.getY() / 32));
            case UP:
                return new PathCords((int) (e.getX() / 32), (int) ((e.getY() + 31) / 32));
            case RIGHT:
            case DOWN:
                return new PathCords((int) (e.getX() / 32), (int) (e.getY() / 32));
    ...
    private boolean isTilesTheSame(PathCords currTile, PathCords newTile) {
        if (currTile.getxCords() == newTile.getxCords())
